...
void init(Data*data){
  ...
  data->program = createProgram(
      compileShader(GL_VERTEX_SHADER  ,loadFile("shaders/interpolation.vp")),
      compileShader(GL_FRAGMENT_SHADER,loadFile("shaders/interpolation.fp")));

  data->mvpUniform = glGetUniformLocation(data->program,"mvp");
  data->modeUniform = glGetUniformLocation(data->program,"mode");
...
void keyDown(SDL_Event event,Data*data){
  if(event.key.keysym.sym == SDLK_q)data->mode^=1;
}
